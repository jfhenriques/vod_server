...
var parseInteger = utils.parseInteger,
	parseFloatNumber = utils.parseFloatNumber;
...
var Server = exports = module.exports = function(options)
{
	...
	this.ifaceRateLimit = ( this.ifaceRate * Server.IFACE_RATE_LIMIT_PERCENT ) | 0 ;
...
Server.getPriorityList = function(listId)
{
	...
	if( listId >= priorityList.length )
		priorityList.length = listId+1;

	if( priorityList[listId] === undefined )
...
Server.setClientFluxRate = function( rate )
{
	Server.CLIENT_FLUX_ESTIMATE = ( 1024 * 1024 * ( parseFloatNumber( rate, 10 ) / 8 ) ) | 0  ;
}
...
Server.initialize = function(options)
{
	...
	Server.IFACE_RATE_LIMIT_PERCENT = parseFloatNumber( options.ifaceLimitRate, 1.0 );

	Server.setClientFluxRate( options.clientDownloadMbits );
}

Server.setUpdateInterval( 30 );
Server.setIfaceTimeout( 5 );
Server.setMetricTimeframe( 60 );
Server.setClientFluxRate( 10 );
Server.USE_PRIORITY_LISTS = true ;
Server.IFACE_RATE_LIMIT_PERCENT = 1.0 ;
...
Server.prototype.canUse = function()
{
	if( this.isValid() )
	{
		var tmp = this.metric;
		...
		if( this.priority > 0 )
			tmp += ( this.priority * Server.CLIENT_FLUX_ESTIMATE );

		return !!( tmp < this.ifaceRateLimit ) ; // force bool
	}
